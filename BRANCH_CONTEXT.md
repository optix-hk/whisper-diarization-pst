...
- Added CLI args: `--speaker-db`, `--match-threshold` (default 0.6), `--interactive`, `--no-persist`
- After diarization, if `--no-persist` is not set and embeddings are available, runs `PersistentSpeakerDiarizer.resolve_speakers()` and reapplies `get_words_speaker_mapping()` with persistent labels
- Backward compatible: `isinstance(diarization_result, DiarizationResult)` check

### diarize_parallel.py

- Same CLI args and integration as `diarize.py`
